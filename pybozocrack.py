...
def crack_single_hash(h):
    ...
    wordlist = response.read().replace('.', ' ').replace(':', ' ').replace('?', '').replace("('", ' ').replace("'",
                                                                                                               ' ').split(
        ' ')
...
class BozoCrack(object):
    def __init__(self, filename, *args, **kwargs):
        ...
        with open(filename, 'r') as f:
            hashes = [h.lower() for line in f if HASH_REGEX.match(line) for h in
                      HASH_REGEX.findall(line.replace('\n', ''))]
